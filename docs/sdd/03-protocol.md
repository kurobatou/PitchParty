...
| `roomState` | `users`, `queue`, `ranking`, `lowLatencyMode`, `nowPlaying`, `mode` | Broadcast a **todos** los sockets conectados cada vez que cambia el estado de la sala. `mode` es el modo de sesión elegido por la Sala (`"karaoke"` \| `"ultrastar"` \| `null`); `nowPlaying` incluye `duetMode`. Ver [04-data-model.md](04-data-model.md) para la forma exacta de cada campo. Los usuarios de rol `karaoke` no tienen socket, así que el broadcast los saltea. |
