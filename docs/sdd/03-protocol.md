...
| `chooseSong` | `songId`, `duetMode?` (`"duo"` \| `"solo"`) | Cantante/invitado | Guarda la canción elegida en el usuario y lo encola (`Room.enqueue`) si `songId` es válido. `duetMode` dice cómo tocar una canción de dos voces (cualquier otro valor se guarda como `null`). **Cambiar de canción deshace cualquier dueto armado sobre la anterior**: se retira la invitación pendiente (`duetInviteCancelled`) o se rompe el acompañamiento (`duetPartnerEnded`), avisándole a la otra persona. Broadcast de `roomState`. |
...
| `setDuetMode` | `duetMode` (`"duo"` \| `"solo"`) | Un celular ya encolado con una canción de dueto | Cambia la elección dúo/solo mientras espera su turno. **Se ignora si lo manda la voz 2** de un dueto con dos celulares: la canción no es suya, está acompañando. Elegir `solo` **cancela** la invitación pendiente o el acompañamiento ya aceptado, y se le avisa a la otra persona (`duetInviteCancelled` / `duetPartnerEnded`, `reason: "hostSolo"`). Broadcast de `roomState`. |
| `inviteDuetPartner` | `toUserId` | Un celular encolado con una canción de dueto en `duo` (modo Karaoke) | Invita a otra persona conectada a cantar la **voz 2** de ese turno. El servidor valida todo (`Room.inviteDuetPartner`) y no confía en la lista que armó el celular: rechaza invitarse a uno mismo (`self`), a alguien que no existe (`unknown`), a la Sala o a un rol `karaoke` (`unreachable`, no tienen socket), a alguien con una invitación pendiente / con compañero / cantando (`busy`), si quien invita no tiene canción (`noSong`) o no está en `duo` (`notDuo`), o si ya tiene compañero (`alreadyPartnered`). Si pasa, le manda `duetInvite` al invitado y `duetInviteSent` a quien invitó; si falla, `duetInviteResult` con `accepted: false` y el motivo. **Invitar a otra persona retira la invitación anterior** (esa recibe `duetInviteCancelled`, `reason: "superseded"`): quien invita tiene una sola canción, así que una sola invitación viva. |
| `respondDuetInvite` | `accept` (bool) | El celular que recibió un `duetInvite` | Acepta o rechaza. Si acepta: se arma el vínculo en los dos sentidos, el invitado pasa a **voz 2**, hereda la canción del titular para poder seguir la letra, y **suelta su propio lugar en la cola** si tenía uno (un dueto es un turno, nadie tiene dos a la vez). Si rechaza: **no** se toca el turno — sigue en `duo` sin compañero. En ambos casos quien invitó recibe `duetInviteResult`. Un `respondDuetInvite` sin invitación pendiente se ignora. Broadcast de `roomState`. |
...
| `roomState` | `users`, `queue`, `ranking`, `lowLatencyMode`, `nowPlaying`, `mode`, `phoneMicEnabled` | Broadcast a **todos** los sockets conectados cada vez que cambia el estado de la sala. `mode` es el modo de sesión elegido por la Sala (`"karaoke"` \| `"ultrastar"` \| `null`); `nowPlaying` incluye `duetMode` y, si el turno es un dueto con dos celulares, `partnerId`/`partnerNickname`. Cada entrada de `queue` lleva `partnerNickname` (o `null`) para que la Sala muestre los dos nombres en **un solo** renglón. Cada usuario lleva `duetPartnerId` y `duetVoice` (1 o 2), que es de donde el celular saca si está en un dueto y cuál voz le toca. `phoneMicEnabled` es el interruptor global de "usar el celular como micrófono" (ver los WebSockets de audio más abajo); los celulares lo usan para decidir si ofrecen el botón. Ver [04-data-model.md](04-data-model.md) para la forma exacta de cada campo. Los usuarios de rol `karaoke` no tienen socket, así que el broadcast los saltea. |
| `duetInvite` | `fromUserId`, `fromNickname`, `songId`, `songTitle` | Solo al celular invitado, cuando alguien manda `inviteDuetPartner`. Abre el modal de aceptar/rechazar. |
| `duetInviteSent` | `toUserId`, `toNickname` | Solo a quien invitó, como acuse: la invitación salió y está esperando respuesta. |
| `duetInviteResult` | `accepted` (bool), `reason`, `partnerId`, `partnerNickname` | Solo a quien invitó. Con `accepted: true` el dueto quedó armado. Con `false`, `reason` dice por qué: `declined` (dijo que no), `gone` (se desconectó antes de contestar), o cualquiera de los motivos de validación de `inviteDuetPartner`. **Un rechazo no cambia el turno**: sigue en `duo` sin compañero. |
| `duetInviteCancelled` | `reason` | Solo al invitado cuya invitación **pendiente** dejó de valer: `superseded` (invitaron a otro), `hostSolo` (se pasó a solista), `hostChangedSong`, `hostLeft` (se desconectó). El celular cierra el modal antes de que alguien acepte algo que ya no existe. |
| `duetPartnerEnded` | `reason` | Solo a la voz 2 de un dueto **ya aceptado** que se deshizo: `hostSolo`, `hostChangedSong`, `partnerLeft` (la voz 2 eligió otra canción), `partnerGone` (se desconectó y venció el período de gracia). Nadie se baja del escenario sin enterarse, igual que nadie sube sin aceptar. |
...
Par de canales que convierten un celular en **micrófono inalámbrico**: su voz sale por los parlantes de la máquina de la Sala. Es un relay tonto — el servidor **nunca decodifica el audio**, solo etiqueta cada frame con la voz que le corresponde y lo reenvía. Existen únicamente para el modo Karaoke, donde `/ws/sing` no se usa porque no hay puntuación.
...
**Regla del turno actual (se aplica en el servidor, no en el celular).** Por **cada frame** recibido, el servidor consulta `Room.micVoiceOf(userId)` y descarta el audio salvo que `phoneMicEnabled` esté activo **y** ese `userId` sea parte del turno actual: `nowPlaying.userId` (voz **1**) o `nowPlaying.partnerId` (voz **2**, el dueto con dos celulares). `canRelayMic(userId)` es simplemente `micVoiceOf(userId) !== null`. Se revisa frame a frame, no al conectar, para que el corte siga al turno cuando cambia: un celular que se quedó con el socket abierto no puede pisar a quien está cantando. Los frames descartados se tiran en silencio (no hay error ni cierre de socket).

### `/ws/micmix` (servidor → Sala)

La Sala abre este socket para **recibir** el audio ya filtrado. Cada frame que sobrevive la regla de arriba se reenvía a todos los oyentes conectados, **con una cabecera de 2 bytes por delante**:

    [ voz (1 o 2) ][ 0 ][ PCM16 mono 16 kHz little-endian … ]

Son **2 bytes y no 1** para que el PCM16 que viene detrás siga alineado a 2 bytes: así la Sala lo lee como una vista `Int16Array` sobre el mismo buffer, sin copiar cada frame. El servidor no toca el audio en sí.

> **Cambio incompatible.** Antes los frames eran PCM crudo sin identificador, porque "solo puede sonar un celular a la vez". Una Sala anterior a este cambio reproduciría la cabecera como audio. Servidor y Sala se actualizan juntos — no hay negociación de versión.

La Sala mantiene **un jitter buffer por voz** (~100 ms cada uno) antes de empezar a reproducir, para absorber saltos de WiFi, y los suma muestra a muestra. Tenerlos separados es justamente lo que permite que una voz se corte —un celular que tartamudea, o un compañero que nunca prendió el micrófono— sin arrastrar a la otra: si un buffer se queda sin audio emite silencio y vuelve a llenarse, mientras la otra voz sigue sonando. Mientras llega audio de cualquiera de las dos, la música baja a `phoneMic.musicVolume` y se restaura cuando deja de llegar (ver `app.js`).

No hay control de volumen por voz: las dos se suman con la misma ganancia y el destino de Web Audio recorta si hace falta. El formato etiquetado lo habilita para más adelante, pero no está implementado.
