...
| `settings.js` | Persiste/lee `server/data/settings.json`: carpetas de biblioteca vigentes, override de IP LAN, dominio público + token de Cloudflare + email para Let's Encrypt. Esto es lo que la UI de Configuración edita en caliente. |
...
| `settings.html` / `settings.js` | Pantalla de **Configuración**: carpetas de biblioteca (con selector nativo o ruta a mano), IP LAN, dominio + token de Cloudflare para Let's Encrypt. Sin rediseño visual propio — hereda la paleta de `style.css` pasivamente. |
